...
processed_data = load_and_process_data('Processed_OnlineSalesData.csv')

# Conduct EDA
# Display basic statistics
basic_stats = processed_data.describe(include='all')
...
for column in processed_data.select_dtypes(include=['float64', 'int64']).columns:
    numeric_distributions[column] = processed_data[column].describe()

# Check unique values in categorical columns
categorical_uniques = {}
for column in processed_data.select_dtypes(include=['object']).columns:
    categorical_uniques[column] = processed_data[column].unique()

basic_stats, head_data, correlation_matrix, numeric_distributions, categorical_uniques